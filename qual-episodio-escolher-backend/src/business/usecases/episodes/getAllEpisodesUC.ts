...
export class GetAllEpisodesUC {
  constructor(private episodeDataSource: EpisodesDataSource) {}
  ...
  public async getAllEpisodesBySerie(
    input: GetOneEpisodeByIdUCInput
  ): Promise<GetLotterysEpisodeOutput> {
    const arrayEpisodes = await this.episodeDataSource.getAllEpisodesBySerie(
      input.idSerie
    );

    const lotteryEpisode =
      arrayEpisodes[Math.floor(Math.random() * arrayEpisodes.length)];

    return {
      episodes: lotteryEpisode && {
